...
class ProductAdmin(admin.ModelAdmin):
    def image_preview(self, obj=None):
        try:
            img = mark_safe("""
            <img style="border-radius:8px; width:120px; height:120px; object-fit: cover; object-position: center; border: 1px solid #ededed;" 
                src="{url}" width="{width}" height={height} />""".format(url = obj.get_image_s, width=120, height=120))
            return img
        except: return '-'
