...
class AttributeValueSerializer(serializers.ModelSerializer):
    ...
    # count2 =             serializers.SerializerMethodField(read_only=True)
    ...
    #     else:
    #         products = Product.objects.filter(product_attrs__value=obj).distinct()
    #     filtered, extra = 0, 0
    #     if params:
...
class ProductSerializer(serializers.ModelSerializer):
    image =        serializers.CharField(label=_('Картинка'), source='get_image_s')
    ...
    class Meta:
        model = Product
        fields = [
            'pk','image','category', 'code', 'customs_code','name', 'price', 'manufacturer', 
            'pieces_in_box','pieces_in_pack','update','has_fda','has_ce','has_description','get_absolute_url','get_databse_url',
...
class ProductSearchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = [
            'image', 'name', 'code', 'price', 'get_absolute_url',
        ]
