...
from django.db.models import Q, Prefetch, Case, When, IntegerField,  BooleanField, Count
...
def catalogue(request, *args, **kwargs):
    ...
    attr_values = AttributeValue.objects.distinct().filter(
        ...
        selected = Case(
            When(pk__in=params['atributes'], then=1),
            default=0,  
...
def export_excel(request, *args, **kwargs):
    ...
    data = [
        ...
        { 'title' : _('Изображение'),                    'width' : 12, 'attr' : 'image'},
            ...
            for j, field in enumerate(data):
                ...
                if 'attr' in field.keys():
                    ...
                    # Image
                        ...
                        ws.insert_image(n, j, settings.BASE_DIR + product.get_image_xs, options)
                        write = False
                    # Price
