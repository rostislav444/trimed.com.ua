...
class Currency(NameSlug):
    code = models.CharField(max_length=100, blank=False, editable=True, verbose_name=_("Код"))
    base = models.BooleanField(default=False, verbose_name=_("Базовая валюта"))
...
class CurrencyExchage(models.Model):
    ...
    value =    models.DecimalField(default=0, decimal_places=4, max_digits=200, verbose_name=_("Обменный курс"))
    date =     models.DateField(default=now)

    class Meta:
        ordering = ('currency',)
        unique_together = ('parent', 'currency',)
        verbose_name = _('Обменный курс')
        verbose_name_plural = _('Обменные курсы')
