...
class PriceFormula(models.Model):
    formula = models.CharField(max_length=255, verbose_name=_("Формула подсчета"), help_text='Цену подставлять в виде английского "x"', validators=[priceFormulaValidator])

    class Meta:
        ordering = ['formula']
        verbose_name = _('Формула цены')
        verbose_name_plural = _('Формулы цен')
...
class Incoterms(NameSlug):
    price =  models.DecimalField(default=10, decimal_places=2, max_digits=200, verbose_name=_("Ставка перевозчика"))
    formula = models.CharField(max_length=255, verbose_name=_("Формула подсчета"), help_text='Цену подставлять в виде английского "x"', validators=[priceFormulaValidator])

    class Meta:
        ordering = ['name']
        verbose_name = _('Инкотермс')
        verbose_name_plural = _('Инкотермс')
...
class InvoiceTemplate(OneFile):
    class Meta:
        verbose_name = _('Шаблон инвойса')
        verbose_name_plural = _('Шаблоны инвойсов')
...
class Invoice(models.Model):
    code =      models.CharField(max_length=255, unique=True, verbose_name=_("Код"))
    ...
    template =  models.ForeignKey(InvoiceTemplate, on_delete=models.PROTECT, verbose_name=_("Шаблон"))
